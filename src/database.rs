...
pub async fn connect(database_url: impl AsRef<str>) -> Result<sqlx::SqlitePool> {
    ...
    let parent = path.parent().ok_or_else(|| {
        Error::new()
            .attach_printable("Invalid database URL: no parent directory found")
            .attach_printable(format!("Database URL: {}", database_url.as_ref()))
    })?;
    if !parent.exists() {
    ...
    sqlx::migrate!("./migrations")
        ...
        .await
        .change_context(Error)
        .attach_printable("Failed to apply sqlx migrations")?;
    Ok(database)
}
